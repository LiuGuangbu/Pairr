...
class photo_http extends StatefulWidget {
  @override
...
class Photo_http extends State<photo_http> {
  List _list = [];
  File path;
  String imageBase;
  ...
  _uploadImage() async {
    ...
    //发送post请求
    var apiUrl = global.get_apiUrlphoto(); //"http://192.168.1.9:5050/photo";
    var response = await http.post(apiUrl, body: {'photo_search': imageBase});
    if (response.statusCode == 200) {
      print("得到数据库数据");
      setState(() {
        print(convert.jsonDecode(response.body)['photo_search']);
